...
public class AddFrame extends JFrame{
	...
	public void saveTask(){
        ...
        for(int i = 0; i < taskList.size(); i++){
        	...
        	if(task.getId() == taskNo){
        		exists = true;
        		break;
        	}
        }
        if(exists){
        	JOptionPane.showMessageDialog(null, "任务编号已存在！", "提示", JOptionPane.ERROR_MESSAGE);
        	return ;
        }
        //检查文件名不能为空
        ...
        for(int i = 0; i < taskList.size(); i++){
        	Task task = taskList.get(i);
        	if(task.getFile().equals(taskFileStr)){
        		exists = true;
        		break;
        	}
        }
        if(exists){
        	JOptionPane.showMessageDialog(null, "保存文件名已存在！", "提示", JOptionPane.ERROR_MESSAGE);
        	return ;
        }
        //检查sql语句不能为空
